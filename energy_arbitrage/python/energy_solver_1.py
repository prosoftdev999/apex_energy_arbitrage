"""Energy solver 1 baseline matching the Rust baselines::energy_solver_1 module."""
...
if TYPE_CHECKING:
    try:
        ...
        from competition.energy_arbitrage.python.challenge import State
        from competition.energy_arbitrage.python.policy_view import PolicyView
...
def _enforce_profit_floor(challenge: "PolicyView", state: "State", action: list[float]) -> list[float]:
    """Scale actions by largest α ∈ [0, 1] such that total_profit + P(α) >= 0, P(α) = α·A − α²·B (match Rust energy_solver_1)."""
    ...
    for i, battery in enumerate(challenge.batteries):
        ...
        a_coeff += u * price * dt - constants.KAPPA_TX * abs(u) * dt
        deg_base = abs(u) * dt / battery.capacity_mwh
        b_coeff += constants.KAPPA_DEG * deg_base * deg_base

    step_profit = a_coeff - b_coeff
    if state.total_profit + step_profit >= 0.0:
        return action

    if b_coeff > EPS:
        disc = a_coeff * a_coeff + 4.0 * b_coeff * state.total_profit
        if disc < 0.0:
            alpha = 0.0
        else:
            alpha = max(0.0, min((a_coeff + disc**0.5) / (2.0 * b_coeff), 1.0))
    elif a_coeff < -EPS:
        alpha = max(0.0, min(state.total_profit / abs(a_coeff), 1.0))
    else:
        alpha = 1.0

    return [u * alpha for u in action]
...
def policy(challenge: "PolicyView", state: "State") -> list[float]:
    """Energy solver 1 policy (matches Rust energy_solver_1::policy)."""

...
def _lp_joint_solve(soc, price, cap, smin, smax, etac, etad, lb, ub, V_next, S,
    ...
    Breakpoints are placed EXACTLY where Vc's slope actually changes (the
    SOC-grid crossings, mapped back into action space separately for the
    charge and discharge branches) rather than at arbitrary evenly-spaced
    action samples -- evenly-spaced sampling can cut a corner of the true
    piecewise-linear Vc whenever a real grid crossing falls between two
    samples. This removes that specific source of approximation error; the
    residual curvature from the (economically negligible) quadratic
    degradation term within any one exact segment is not addressed by this
    change, since it is many orders of magnitude smaller than price terms.
    """
    B = len(soc)
    L = ptdf_np.shape[0]
    ...
    for i in range(B):
        ...
        soc_grid_i = smin[i] + span_i * (np.arange(S) / (S - 1))

        # Exact breakpoints: place them where the continuation value's slope
        # actually changes (the SOC grid crossings), not at arbitrary evenly
        # spaced action samples -- an evenly-spaced sampling can cut a corner
        # of the true piecewise-linear Vc whenever a grid crossing falls
        # between two samples. Charge and discharge map to SOC oppositely, so
        # each branch is inverted separately.
        us_list = [lo, 0.0, hi]
        if hi > 1e-12:  # discharge branch: new_soc = soc - u*dt/etad, decreasing in u
            u_from_grid = (soc[i] - soc_grid_i) * etad[i] / _DT
            us_list.extend(u_from_grid[(u_from_grid > 0.0) & (u_from_grid < hi)].tolist())
        if lo < -1e-12:  # charge branch: new_soc = soc - etac*u*dt, increasing as u decreases
            u_from_grid = (soc[i] - soc_grid_i) / (etac[i] * _DT)
            us_list.extend(u_from_grid[(u_from_grid < 0.0) & (u_from_grid > lo)].tolist())
        us = np.unique(np.clip(np.array(us_list), lo, hi))
        if len(us) < 2:
            us = np.array([lo, hi])
